#!/usr/bin/env python3
"""Compute F-LMD from paired GT/generated 68-point facial landmarks."""
...
from metrics.landmark_distance import FACE_68, landmark_distance


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compute F-LMD from paired landmarks.")
...
def main() -> None:
    ...
    for gt_path, gen_path in paired_files(args.gt_dir, args.generated_dir, LANDMARK_EXTS):
        score = landmark_distance(load_landmarks(gt_path), load_landmarks(gen_path), indices=FACE_68)
        scores.append(score)
        print(f"{gt_path.name}: {score:.6f}")
    result = {"f_lmd": float(sum(scores) / len(scores)), "num_pairs": len(scores)}
    print(f"F-LMD: {result['f_lmd']:.6f}")
