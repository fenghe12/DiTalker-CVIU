...
def save_videos_grid(videos: torch.Tensor, path: str, rescale=False, n_rows=6, fps=12, imageio_backend=True, color_transfer_post_process=False, audio_path : str =None,output_path : str =None):
    ...
    if audio_path is not None:
        import subprocess
        subprocess.run(['ffmpeg', '-y', '-i', path, '-i', audio_path, '-c:v', 'copy', '-c:a', 'aac', '-shortest', output_path])
        os.remove(path)

        pass


def save_videos_grid_batch(videos: torch.Tensor, path: str, rescale=False, n_rows=6, fps=12, imageio_backend=True, color_transfer_post_process=False, audio_path : str =None,output_path : str =None):
    videos = rearrange(videos, "b c t h w -> t b c h w")
    outputs = []
    for x in videos:
        x = torchvision.utils.make_grid(x, nrow=n_rows)
