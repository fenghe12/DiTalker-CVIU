...
from ..models.base.transformer3d import Transformer3DModel
...
class EasyAnimateInpaintPipeline(DiffusionPipeline):
    ...
    def decode_latents(self, latents,is_training=False):
        ...
        video = video.cpu().float().numpy() if not is_training else video
        return video
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """
        # assert audio_encoder_hidden_states is not None, "`audio_encoder_hidden_states` must be provided"
        ...
        if audio_encoder_hidden_states is not None:
            if audio_encoder_hidden_states.ndim == 3:
                audio_encoder_hidden_states = audio_encoder_hidden_states  # already (B, F, ...)
            elif audio_encoder_hidden_states.ndim == 4:
                pass  # (B, F, 50, 384)
            else:
                audio_encoder_hidden_states = audio_encoder_hidden_states.unsqueeze(0)
            audio_encoder_hidden_states = audio_encoder_hidden_states[:,:video_length].to(latents.device, latents.dtype)
            if do_classifier_free_guidance:
                audio_uncond = torch.zeros_like(audio_encoder_hidden_states)
                audio_encoder_hidden_states = torch.cat([audio_uncond, audio_encoder_hidden_states])
        if phoneme_win is not None:
            phoneme_win = phoneme_win.unsqueeze(0) if phoneme_win.ndim == 2 else phoneme_win
            phoneme_win = phoneme_win[:,:video_length].to(latents.device)
            style_clip = style_clip.unsqueeze(0) if style_clip.ndim == 2 else style_clip
            style_clip = style_clip.to(latents.device, latents.dtype)
            pad_mask = pad_mask.unsqueeze(0) if pad_mask.ndim == 1 else pad_mask
            pad_mask = pad_mask.to(latents.device)
            if do_classifier_free_guidance:
                phoneme_win = torch.cat([torch.zeros_like(phoneme_win), phoneme_win])
                style_clip = torch.cat([torch.zeros_like(style_clip), style_clip])
                pad_mask = torch.cat([torch.ones_like(pad_mask), pad_mask])  # all-True mask for uncond
        if pixel_values_pose is not None:
            pixel_values_pose = pixel_values_pose.to(latents.device, latents.dtype)
            if do_classifier_free_guidance:
                pixel_values_pose = torch.cat([torch.zeros_like(pixel_values_pose), pixel_values_pose])

        # c_emo CFG: uncond = zeros
        if c_emo is not None:
            c_emo = c_emo.to(latents.device, latents.dtype)
            if do_classifier_free_guidance:
                c_emo = torch.cat([torch.zeros_like(c_emo), c_emo])
            ...
            for i, t in enumerate(timesteps):
                ...
                with torch.no_grad():
                    ...
                    transformer_inputs = (
                        ...
                        clip_attention_mask_actual_input,
                    ...
                    # 定义自定义调用函数
                    def custom_transformer_call(transformer_model, *args):
                        # 解包参数
                        (latent, embeds, mask, timestep, cond_kwargs, 
                        inpaint, clip_embeds, clip_mask, audio, 
                        phoneme, style, pad, pose, return_dict) = args
                        ...
                        return transformer_model(
                            ...
                            clip_encoder_hidden_states=clip_embeds,
                            clip_attention_mask=clip_mask,
                            audio_encoder_hidden_states=audio,
                            phoneme_win=phoneme,
                            style_clip=style,
                            pad_mask=pad,
                            pixel_values_pose=pose,
                            return_dict=return_dict
                        )[0]  # 注意这里取第一个输出，与原始代码一致
                    
                    # 计算 FLOPs
                    flops, params = profile(
                        self.transformer,
                        inputs=transformer_inputs,
                        custom_ops={self.transformer: custom_transformer_call},
                        verbose=False
                    )
                    
                    # 累加 FLOPs
                    denoising_flops += flops
                    print(f"单次Transformer前向传播 FLOPs: {flops / 1e9:.2f} GFLOPs")
    ...
    def decode_latents_train(
        ...
        **kwargs,
        ...
        video = self.decode_latents(latents,is_training=True)
