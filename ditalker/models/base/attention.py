...
class TemporalTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.enable_style_attn:
            self.style_attention = Attention(
                    query_dim=dim,
                    cross_attention_dim=cross_attention_dim if not double_self_attention else None,
                    heads=num_attention_heads,
                    dim_head=attention_head_dim,
            ...
            self.norm_style = None

            # print("hahahahha")
            # input("test")

        # Expression Cross-Attention (ECA) layer (Paper Sec 3.2, Eq.5)
        # ECA(z^i, c_emo) = softmax(z^i W_Q^E (c_emo W_K^E)^T / sqrt(d)) * c_emo W_V^E
        # c_emo = cat(c_text, c_ref) where c_text is T5-encoded expression prompt,
        # c_ref is CLIP-encoded reference face embedding.
        # Placed after ASFM, before FFN in each DiT block.
        if self.enable_eca:
            if pkg_resources.parse_version(installed_version) >= pkg_resources.parse_version("0.28.2"):
                self.emotion_cross_attention = Attention(
                    query_dim=dim,
                    cross_attention_dim=cross_attention_dim if not double_self_attention else None,
                    heads=num_attention_heads,
                    dim_head=attention_head_dim,
            ...
            self.emotion_cross_attention = None
            self.norm_eca = None
    ...
    def forward(
        ...
        audio_encoder_hidden_states: Optional[torch.FloatTensor] = None,
        style_encoder_hidden_states: Optional[torch.FloatTensor] = None,
        scale_scalar: Optional[torch.FloatTensor] = None,
        audio_scale_scalar: Optional[torch.FloatTensor] = None,
        c_emo: Optional[torch.FloatTensor] = None,
        layer_index = 0,
        num_frames: int = 16,
        height: int = 32,
        width: int = 32,
        ...
        if self.attn_temporal is not None:
            num_frames_temp=hidden_states.shape[1]/width
            num_frames_temp /=height
            num_frames_temp = int(num_frames_temp)
            attn_output = rearrange(hidden_states, "b (f h w) c -> b c f h w", f=num_frames_temp, h=height, w=width)
        ...
        if self.attn2 is not None:
            ...
            # ============================================================
            # ASFM: Parallel Audio-Style Fusion Module (Paper Sec 3.2, Eq.4)
            # z^{i+1} = z^i + s_alpha * ACA(z^i, c_a) + s_phi * SCA(z^i, c_s)
            # Both ACA and SCA use the SAME pre-update z^i as query.
            # ============================================================
            asfm_residual = hidden_states  # Save z^i for parallel fusion

            # ACA branch: Audio Cross-Attention
            audio_attn_output = torch.zeros_like(hidden_states)
            if self.enable_audio_attention:
                if self.use_ada_layer_norm:
                    audio_norm_hidden_states = self.norm_audio(asfm_residual, timestep)
                elif self.use_ada_layer_norm_zero or self.use_layer_norm:
                    audio_norm_hidden_states = self.norm_audio(asfm_residual)
                elif self.use_ada_layer_norm_single:
                    audio_norm_hidden_states = asfm_residual
                else:
                    raise ValueError("Incorrect norm")

                if self.pos_embed is not None and self.use_ada_layer_norm_single is None:
                    audio_norm_hidden_states = self.pos_embed(audio_norm_hidden_states)

                if audio_norm_hidden_states.dtype != encoder_hidden_states.dtype:
                    audio_norm_hidden_states = audio_norm_hidden_states.to(encoder_hidden_states.dtype)

                audio_attn_output = self.audio_attention(
                    audio_norm_hidden_states,
                    encoder_hidden_states=audio_encoder_hidden_states,
                    attention_mask=None,
                    **cross_attention_kwargs,
                )
                # Scale by s_alpha from ScaleAdapter
                if self.enable_audio_scale_fusion and audio_scale_scalar is not None:
                    audio_scale_scalar = audio_scale_scalar.to(audio_attn_output.device)
                    audio_attn_output = audio_attn_output * audio_scale_scalar[:, layer_index, :].unsqueeze(1)

            # SCA branch: Style Cross-Attention (uses same z^i, NOT audio-updated)
            style_attn_output = torch.zeros_like(hidden_states)
            if self.enable_style_attn:
                if self.use_ada_layer_norm:
                    style_norm_hidden_states = self.norm_style(asfm_residual, timestep)
                elif self.use_ada_layer_norm_zero or self.use_layer_norm:
                    style_norm_hidden_states = self.norm_style(asfm_residual)
                elif self.use_ada_layer_norm_single:
                    style_norm_hidden_states = asfm_residual
                else:
                    raise ValueError("Incorrect norm")

                if self.pos_embed is not None and self.use_ada_layer_norm_single is None:
                    style_norm_hidden_states = self.pos_embed(style_norm_hidden_states)

                if style_norm_hidden_states.dtype != encoder_hidden_states.dtype:
                    style_norm_hidden_states = style_norm_hidden_states.to(encoder_hidden_states.dtype)

                style_attn_output = self.style_attention(
                    style_norm_hidden_states,
                    encoder_hidden_states=style_encoder_hidden_states,
                    attention_mask=None,
                    **cross_attention_kwargs,
                )
                # Scale by s_phi from ScaleAdapter
                if self.enable_scale_fusion and scale_scalar is not None:
                    style_attn_output = style_attn_output * scale_scalar[:, layer_index, :].unsqueeze(1)

            # Parallel fusion: z^{i+1} = z^i + s_alpha*ACA + s_phi*SCA
            hidden_states = asfm_residual + audio_attn_output + style_attn_output

            # ============================================================
            # ECA: Expression Cross-Attention (Paper Sec 3.2, Eq.5)
            # z = z + ECA(norm(z), c_emo), placed after ASFM, before FFN
            # ============================================================
            if self.enable_eca and self.emotion_cross_attention is not None and c_emo is not None:
                if self.use_ada_layer_norm:
                    eca_norm_hidden_states = self.norm_eca(hidden_states, timestep)
                elif self.use_ada_layer_norm_zero or self.use_layer_norm:
                    eca_norm_hidden_states = self.norm_eca(hidden_states)
                elif self.use_ada_layer_norm_single:
                    eca_norm_hidden_states = hidden_states
                else:
                    raise ValueError("Incorrect norm")

                if eca_norm_hidden_states.dtype != c_emo.dtype:
                    eca_norm_hidden_states = eca_norm_hidden_states.to(c_emo.dtype)

                eca_output = self.emotion_cross_attention(
                    eca_norm_hidden_states,
                    encoder_hidden_states=c_emo,
                    attention_mask=None,
                    **cross_attention_kwargs,
                )
                hidden_states = hidden_states + eca_output
...
class SelfAttentionTemporalTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if self.attn2 is not None:
            ...
            if self.enable_audio_attention:
                if self.use_ada_layer_norm:
                    audio_norm_hidden_states = self.norm_audio(hidden_states, timestep)
                elif self.use_ada_layer_norm_zero or self.use_layer_norm:
                    audio_norm_hidden_states = self.norm_audio(hidden_states)
                elif self.use_ada_layer_norm_single:
                # For PixArt norm2 isn't applied here:
                # https://github.com/PixArt-alpha/PixArt-alpha/blob/0f55e922376d8b797edd44d25d0e7464b260dcab/diffusion/model/nets/PixArtMS.py#L70C1-L76C103
                    audio_norm_hidden_states = hidden_states
                else:
                    raise ValueError("Incorrect norm")
                ...
                if audio_norm_hidden_states.dtype != encoder_hidden_states.dtype or audio_norm_hidden_states.dtype != encoder_attention_mask.dtype:
                    audio_norm_hidden_states = audio_norm_hidden_states.to(encoder_hidden_states.dtype)
                ...
                hidden_states = hidden_states + audio_attn_output
...
class KVCompressionTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        audio_encoder_hidden_states : Optional[torch.LongTensor] = None,
    ) -> torch.FloatTensor:
        # Notice that normalization is always applied before the real computation in the following blocks.
        # 0. Self-Attention
        batch_size = hidden_states.shape[0]
