...
class ScaleEncoder(nn.Module):
    def __init__(self, in_dim=1152, out_dim=1, num_heads=8, num_tokens=16, n_layers=2):
        super().__init__()
        scale = in_dim ** -0.5
        self.num_tokens = num_tokens
        self.scale_emb = nn.Parameter(torch.randn(1, num_tokens, in_dim) * scale)
        self.transformer_blocks = ArchTransformer(
                width=in_dim,
                layers=n_layers,
                heads=num_heads,
            )
...
if __name__=="__main__":
    model =ScaleEncoder(num_tokens=28).cuda()
    input = torch.randn(2, 128,1152).cuda()
