...
# kwargs_accelerate = DDPK(find_unused_parameters=True)
import diffusers
import numpy as np
import torch
...
from ditalker.data.bucket_sampler import (ASPECT_RATIO_512,
                                             ASPECT_RATIO_RANDOM_CROP_512,
                                             ASPECT_RATIO_RANDOM_CROP_PROB,
                                             AspectRatioBatchImageSampler,
                                             AspectRatioBatchImageVideoSampler,
                                             RandomSampler, get_closest_ratio)
from ditalker.data.dataset_image import CC15M
from ditalker.models.base.Syncnet import SyncNetPerception
from ditalker.models.base.color_syncnet import SyncNet_color
from ditalker.data.dataset_image_video import (ImageVideoDataset,
                                                  ImageVideoSampler,
                                                  get_random_mask)
from ditalker.models.base.autoencoder_magvit import AutoencoderKLMagvit
from ditalker.models.base.transformer3d import (HunyuanTransformer3DModel,
                                              Transformer3DModel)
from ditalker.pipeline.pipeline_easyanimate_multi_text_encoder import EasyAnimatePipeline_Multi_Text_Encoder
from ditalker.pipeline.pipeline_easyanimate_multi_text_encoder_inpaint import EasyAnimatePipeline_Multi_Text_Encoder_Inpaint
from ditalker.pipeline.pipeline_easyanimate import EasyAnimatePipeline
from ditalker.pipeline.pipeline_easyanimate_inpaint import \
    EasyAnimateInpaintPipeline
from ditalker.pipeline.pipeline_easyanimate_multi_text_encoder import (
    get_2d_rotary_pos_embed, get_resize_crop_region_for_grid)
from ditalker.pipeline.pipeline_pixart_magvit import \
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="checkpoints/syncnet_128mouth.pth",
        required=False,
        help="Path to pretrained syncnet model",
    )
    parser.add_argument(
        "--pretrained_color_syncnet_path",
        type=str,
        default="checkpoints/syncnet_color.pth",
        required=False,
        help="Path to pretrained syncnet model",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--enable_audio_emb",
        action="store_true",
        help="whether to enable audio attention layers",
    )
    parser.add_argument(
        "--enable_region_mask_loss",
        action="store_true",
        help="whether to enable lip and eyes masked loss",
    )
    parser.add_argument(
        "--enable_repa",
        action="store_true",
        help="whether to enable repa loss",
    )
    parser.add_argument(
        "--enable_sync_lip_loss",
        action="store_true",
        help="whether to enable sync lip loss",
    )
    parser.add_argument(
        "--sync_lip_loss_weight",
        type=float,
        default=0.1,
        help="whether to enable sync lip loss",
    )
    parser.add_argument(
        "--enable_gan_loss",
        action="store_true",
        help="whether to enable gan loss",
    )
    parser.add_argument(
        "--enable_lpips_loss",
        action="store_true",
        help="whether to enable lpips loss",
    )

    parser.add_argument(
        "--enable_scale_fusion",
        action="store_true",
        help="whether to scaled fusion",
    )
    parser.add_argument(
        "--enable_scale_time_embedding",
        action="store_true",
        help="whether to enable scale time embedding",
    )
    # parser.add_argument(
    #     "--audio_emb_dir",
    #     type=str,
    #     default=None,
    #     help="The directory where contains pre-processed audio embeddings.(whisper embeddings)",
    # )
    parser.add_argument(
        "--use_came",
        action="store_true",
    ...
    parser.add_argument(
        "--enable_style_attn", action="store_true", help="Whether enable style attention brance."
    )
    parser.add_argument(
        "--enable_pose_adapter", action="store_true", help="Whether enable pose guidance adapter."
    )
    parser.add_argument(
        "--finetune_emo", action="store_true", help="Whether enable emotion finetuning."
    )
    parser.add_argument(
        "--enable_eca", action="store_true", help="Whether enable emotion-conditioned attention (Phase3)."
    )
    parser.add_argument(
        "--motion_sub_loss_ratio", type=float, default=0.25, help="The ratio of motion sub loss."
    )
    parser.add_argument(
        "--region_loss_weight", type=float, default=10, help="The weight of eyes and lip region loss."
    )
    parser.add_argument(
        "--text_drop_ratio", type=float, default=-1, help="Text condition drop ratio. Negative values use stage defaults."
    )
    ...
    parser.add_argument(
        "--pretrained_i2v_weight",
        type=str,
        default=None,
        help=("Path to pretrained i2v model weight."),
    )
    parser.add_argument(
        "--vae_path",
        type=str,
    ...
    parser.add_argument(
        "--only_train_audio", action="store_true", help="Whether enable only train audio mode."
    )
    parser.add_argument(
        "--zero_init_newly_added_modules", action="store_true", help="Whether zero init newly added module."
    )
    parser.add_argument(
        "--train_mode",
        type=str,
    ...
    # print(args.enable_audio_emb)
    # print(args.audio_emb_dir)
...
def apply_config_overrides(args, config):
    ...
    if resolution is not None:
        args.video_sample_size = resolution
        args.image_sample_size = resolution

    sample_stride = config_dict.get("sample_stride")
    if sample_stride is not None:
        args.video_sample_stride = sample_stride

    video_length = config_dict.get("video_length")
    if video_length is not None:
        args.video_sample_n_frames = video_length

    save_steps = config_dict.get("save_steps")
    if save_steps is not None:
        args.checkpointing_steps = save_steps

    trainable_modules = config_dict.get("trainable_modules")
    if trainable_modules is not None:
        if isinstance(trainable_modules, str):
            args.trainable_modules = [trainable_modules]
        else:
            args.trainable_modules = trainable_modules

    zero_init = config_dict.get("zero_init")
    if zero_init is not None:
        args.zero_init_newly_added_modules = zero_init

    transformer_additional_kwargs = config_dict.get("transformer_additional_kwargs")
    if isinstance(transformer_additional_kwargs, dict):
        for key in ("enable_style_attn", "enable_pose_adapter", "enable_eca"):
            value = transformer_additional_kwargs.get(key)
            if value is not None:
                setattr(args, key, value)
...
def main():
    ...
    if args.enable_sync_lip_loss:

        # sync_net = SyncNetPerception(args.pretrained_syncnet_path).to(device=accelerator.device,dtype=weight_dtype)
        sync_net=SyncNet_color().to(device=accelerator.device,dtype=weight_dtype)
        sync_net.load_state_dict(torch.load(args.pretrained_color_syncnet_path,map_location="cpu"),strict=False)
    else:
        sync_net =None
    ...
    if args.enable_sync_lip_loss:
        sync_net.requires_grad_(False)
    if config.get('enable_multi_text_encoder', False):
        text_encoder_2.requires_grad_(False)
    transformer3d.requires_grad_(False)
    if args.enable_repa:
        enc_type = 'dinov2-vit-b'
        encoders, encoder_types, architectures = load_encoders(enc_type, accelerator.device)
        encoders[0].to(device=accelerator.device,dtype=weight_dtype)
        encoders[0].requires_grad_(False)
        # print(encoders[0])
    ...
    for name, param in transformer3d.named_parameters():
        for trainable_module_name in args.trainable_modules + args.trainable_modules_low_learning_rate:
            if trainable_module_name in name:
                param.requires_grad = True if not args.only_train_audio else False
                print(f"{name} requires grad")
                break
    
    if args.only_train_audio:
        for name, param in transformer3d.named_parameters():
            if 'audio' in name or 'style'  in  name    or "scale_adapter" in name :
                param.requires_grad = True
                print(f"{name} requires grad only train audio")

    if args.enable_pose_adapter:
        for name, param in transformer3d.named_parameters():
            if 'pose' in name  :
                param.requires_grad = True
                print(f"{name} requires grad for pose")
    if args.zero_init_newly_added_modules:
        import re
        pattern = re.compile(r'transformer_blocks\.\d+\.(audio|style)_attention\.to_out\.\d+\.(weight|bias)')
        for name, param in transformer3d.named_parameters():
            if pattern.match(name):
                with torch.no_grad():  # 在无梯度上下文中修改参数
                    param.zero_()  # 将参数初始化为零
                    print(f"{name} has been zero-initialized")
    ...
    train_dataset = ImageVideoDataset(
        ...
        enable_audio_emb=args.enable_audio_emb,
        text_drop_ratio=args.text_drop_ratio,
        enable_style_attn=args.enable_style_attn,
        enable_pose_adapter=args.enable_pose_adapter,
        enable_sync_lip_loss=args.enable_sync_lip_loss,
        finetune_emo = args.finetune_emo,
    )
    train_dataset.__getitem__(111)
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            if args.train_mode != "normal":
                ...
                new_examples["audio_encoder_hidden_states"] = []
                new_examples["eye_mask_values"] = []
                new_examples["lip_mask_values"] = []
                new_examples["full_mask_values"] = []
                new_examples["phoneme_win"] = []
                new_examples["style_clip"] = []
                new_examples["pad_mask"] = []
                new_examples["pixel_values_pose"] = []
                new_examples['deepspeech_emb'] = []
                new_examples['mel_emb'] = []
            ...
            for example in examples:
                ...
                new_examples["audio_encoder_hidden_states"].append(example["audio_encoder_hidden_states"])
                new_examples["eye_mask_values"].append(example["eye_mask_values"])
                new_examples["lip_mask_values"].append(example["lip_mask_values"])
                new_examples["full_mask_values"].append(example["full_mask_values"])
                new_examples["phoneme_win"].append(example["phoneme_win"]) 
                new_examples["style_clip"].append(example["style_clip"]) 
                new_examples["pad_mask"].append(example["pad_mask"]) 
                new_examples["pixel_values_pose"].append(example["pixel_values_pose"])
                new_examples['deepspeech_emb'].append(example["deepspeech_emb"])
                new_examples["mel_emb"].append(example["mel_emb"])
            ...
            if args.enable_audio_emb:
                
                new_examples["audio_encoder_hidden_states"] = torch.stack([example[:batch_video_length] for example in new_examples["audio_encoder_hidden_states"]])
                new_examples["eye_mask_values"]=torch.stack([example[:16] for example in new_examples["eye_mask_values"]])
                new_examples["lip_mask_values"]=torch.stack([example[:16] for example in new_examples["lip_mask_values"]])
                new_examples["full_mask_values"]=torch.stack([example[:16] for example in new_examples["full_mask_values"]])

            if args.enable_style_attn:

                new_examples["phoneme_win"] = torch.stack([example[:batch_video_length] for example in new_examples["phoneme_win"]])
                new_examples["style_clip"] = torch.stack([example[:] for example in new_examples["style_clip"]])
                new_examples["pad_mask"] = torch.stack([example[:] for example in new_examples["pad_mask"]])
                # print(new_examples["audio_encoder_hidden_states"].shape)
                # print(new_examples["pixel_values"].shape)
            if args.enable_pose_adapter:
                # print(batch_video_length)
                new_examples["pixel_values_pose"] = torch.stack([example[:batch_video_length] for example in new_examples["pixel_values_pose"]])

            if args.enable_sync_lip_loss:
                new_examples['deepspeech_emb'] = torch.stack([example[:] for example in new_examples['deepspeech_emb'] ])
                new_examples['mel_emb'] = torch.stack([example[:] for example in new_examples['mel_emb'] ])
    ...
    # random_sample = random.choice(batch['pixel_values']) 
    # for batch in train_dataloader:
    # # 假设 'pixel_values' 是你想要从中选择随机样本的键
    #     random_sample = random.choice(batch['pixel_values'])
    #     print(random_sample.shape)
    #     break  # 仅从第一个batch中获取样本
    ...
    if args.pretrained_i2v_weight is not None:
        from safetensors.torch import load_file
        loaded_tensors = load_file(args.pretrained_i2v_weight)
        transformer3d.load_state_dict(loaded_tensors,strict=False)
        print(f"loaded i2v weight from {args.pretrained_i2v_weight}")
    ...
    # print(pipeline)
    # temp_latents = torch.randn(1,4,16,64,64).to(accelerator.device, dtype=weight_dtype)
    # print(pipeline.decode_latents_train(latents=temp_latents).videos)
    ...
    repa_transform = transforms.Compose([
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                audio_encoder_hidden_states = batch["audio_encoder_hidden_states"].to(weight_dtype)
                pixel_values_pose = batch["pixel_values_pose"].to(weight_dtype).transpose(
                    1, 2
                )  # (bs, c, f, H, W))
                # print(pixel_values_pose.shape)


                ###don't convert to weight_dtype !!!!!!

                # style_clip=batch["style_clip"].to(weight_dtype)
                # phoneme_win=batch["phoneme_win"].to(weight_dtype)
                # pad_mask=batch["pad_mask"].to(weight_dtype)
                style_clip=batch["style_clip"]
                phoneme_win=batch["phoneme_win"]
                pad_mask=batch["pad_mask"]
                ###we should convert them after the forward pass of style/content encoder

                # print(batch.keys())
                region_mask={}
                region_mask["lip_region_mask"] =batch["lip_mask_values"].to(weight_dtype)
                # print("region",region_mask["lip_region_mask"].shape)
                # print(region_mask["lip_region_mask"].shape)
                region_mask ["eye_region_mask"]= batch["eye_mask_values"].to(weight_dtype)
                region_mask ["face_region_mask"] = batch["full_mask_values"].to(weight_dtype)
                if args.enable_sync_lip_loss:
                    deepspeech_emb = batch["deepspeech_emb"].to(weight_dtype)
                    mel_emb = batch["mel_emb"].to(weight_dtype)
                else:
                    deepspeech_emb=None
                    ...
                    if args.enable_repa:
                        ...
                        with torch.no_grad():
                            ...
                            for clip_pixel_value in clip_pixel_values:
                                ...
                                # image=clip_pixel_value
                                inputs = repa_transform(image).to(accelerator.device, dtype=weight_dtype)
                                # print(inputs.shape)
                                # print(inputs)
                                # inputs=preprocess_raw_image(image,enc_type='dinov2-vit-b')

                                # inputs = image_processor(images=image, return_tensors="pt")
                                # inputs["pixel_values"] = inputs["pixel_values"].to(accelerator.device, dtype=weight_dtype)
                                # print(inputs["pixel_values"])
                                # print(inputs)
                                # z = encoders[0].forward_features(inputs["pixel_values"])
                                z = encoders[0].forward_features(inputs.unsqueeze(0))
                                if 'dinov2' in enc_type: z = z['x_norm_patchtokens']
                                # print(z.shape)
                                # outputs = image_encoder(**inputs).image_embeds[0]
                                repa_z.append(z)
                            z = torch.stack(repa_z).squeeze(1)
                            # print(z.shape)
                            # print(z.shape)
                            # bsc = clip_encoder_hidden_states.shape[0]
                        #     raw_image = clip_pixel_values.tranpose(1,3)
                        # print(clip_pixel_values.shape)
                        # raw_image_ = preprocess_raw_image(raw_image, enc_type)
                        # z = encoders[0].forward_features(raw_image_)
                        # if 'dinov2' in enc_type: z = z['x_norm_patchtokens']
                        # print(z.shape)
                    else:
                        bsz = 6
                        z =torch.zeros(bsz,256,768)
                ...
                if config.get('enable_multi_text_encoder', False):
                    ...
                    loss_term = train_diffusion.training_losses(
                        ...
                        model_kwargs=dict(
                            ...
                            pad_mask=pad_mask,
                            c_emo=None,  # Phase3: placeholder for emotion conditioning
                            pixel_values_pose=pixel_values_pose,


                        ),
                        enable_region_mask_loss=args.enable_region_mask_loss,
                        region_loss_weight=args.region_loss_weight,
                        region_mask=region_mask,
                        enable_repa = args.enable_repa,
                        z = z,
                        repa_weight = args.repa_weight,
                        deepspeech_emb=deepspeech_emb,
                        sync_net=sync_net,
                        sync_lip_loss_weight=args.sync_lip_loss_weight,
                        pipeline=pipeline,
                        mel_emb = mel_emb if args.enable_sync_lip_loss else None,
                    )
